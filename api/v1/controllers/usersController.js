...
export const sendFriendRequest = async (req, res, next) => {
  try {
    ...
    // you cannot send a friend request to yourself, to someone who have blocked you, to some who you are already friends with and you cannot send request twice
    if (!user || !friend) {
      throw new NotFoundError('no use with the provided id!');
    }
    if (friendID === userID) {
    ...
    if (friend.blockedUsers.includes(userID)) {
      throw new BadRequestError(
        `you cannot send a friend request because  @${user.name} blocked you `
      );
    }
